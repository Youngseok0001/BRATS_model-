...
import nets.convLSTM_upgrade as convLSTM
import copy
...
class Model(object):
    ...
    def net(self, inputs, is_training=True):
        ...
        start_channel = 40#[40 kernels]
        batch_size, seq_len, _, _, _ = inputs.get_shape().as_list()#[3,3,H,W,4]
        end_points = {}#?
        images_seq = tf.transpose(inputs, [1, 0, 2, 3, 4])#[bring sequence forward]
        images_seq = tf.unstack(images_seq)#convert to 4 dimensional mtx [3*3,H,W,4]
